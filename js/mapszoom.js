...
//console.log(data);
    ...
    for (i = purban+1; i<purban+prural; i++) {
        bs[i] = data[i].ID;
    }
    var kecamatan = [];
...
/* var projection = d3.geo.mercator()
    .scale(100782.32946559653)
    .center([102.6381,-3.6138]) //projection center
    .translate([width/2,height/2]) //translate to center the ma0p in view */
var p=0;
if(pilihank[sele][0]=="Rejanglebong"){
	var skala=2.3;
	var p=25;
}else if(pilihank[sele][0]=="Bengsel"){
	var skala=2.2;
	var p=-23;
}else if(pilihank[sele][0]=="Benteng"){
	var skala=2.4;
	var p=17;
}else if(pilihank[sele][0]=="Bengut"){
	var skala=1.2;
	var p=-5;
}else if(pilihank[sele][0]=="Kaur"){
	var skala=1.34;
	var p=5;
}else if(pilihank[sele][0]=="Kepahiang"){
	var skala=2.83;
	var p=-58;
}else if(pilihank[sele][0]=="Kotabeng"){
	var skala=4.1;
	var p=-37;
}else if(pilihank[sele][0]=="Lebong"){
	var skala=1.35;
	var p=76;
}else if(pilihank[sele][0]=="Mukomuko"){
	var skala=1;
	var p=-30;
}else if(pilihank[sele][0]=="Seluma"){
	var skala=1.5;
	var p=-30;
}else{
	var skala=2;
...
var projection = d3.geo.mercator()
    .scale(window.innerWidth*30*skala)
...
//Create an SVG
	...
	var svg = d3.selectAll(".mapzoom"+a).append("svg")
		.attr("width", width)
		.attr("height", height);
...
var tooltip = d3.selectAll(".mapzoom"+a).append("div").attr("class","tooltip");
