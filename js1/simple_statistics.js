...
(function() {
    ...
    ss.jenks = function(data, n_classes) {
        ...
        while (countNum > 1) {
            kclass[countNum - 1] = roundNum(data[lower_class_limits[k][countNum] - 2]);
            k = lower_class_limits[k][countNum] - 1;
            countNum--;
            
            //modif by wida
            if (data[lower_class_limits[k][countNum]-1 ] != 0){
                arrNilai.push(roundNum(data[lower_class_limits[k][countNum] ]));
            }
            
            //console.log( data[lower_class_limits[k][countNum]-1 ]);
            //console.log(roundNum(data[lower_class_limits[k][countNum]-1 ]))
            //console.log(numDigits(data[lower_class_limits[k][countNum]-1 ]))
            //console.log(Math.round( ( data[lower_class_limits[k][countNum]-1 ]) / 100) * 100);
            //console.log( data[lower_class_limits[k][countNum] ]);
        }
        ...
        function roundNum( number){
            var hasil;
            var digit = numDigits(number)
            if (digit == 3){
                hasil = Math.round( number / 10) * 10
            }else if (digit > 3 ) {
                hasil = Math.round( number / 100) * 100
            }else{
                hasil = number
            }
            return hasil
        ...
        //};
        //MinNilai =Array.min(arrNilai)
        kclass[1]= arrNilai[arrNilai.length-1] // ambil array terendah dari bawah
        
        //kclass= [701936, 1089565, 3440573, 6621831, 25516999]
        kclass= kclass.slice(1)
        kclass[kclass.length-1] = kclass[kclass.length-1] + 1;
